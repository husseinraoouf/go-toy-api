...
func CreateDeck(w http.ResponseWriter, r *http.Request) {
	...
	deck, err := repo.CreateDeck(repo.CreateDeckOptions{
		Shuffled: input.Shuffled,
		Cards:    input.Cards,
	})
	if err != nil {
		if repo.IsErrInvalidCardCode(err) ||
			repo.IsErrDuplicateCardCode(err) {
	...
	c.JSON(http.StatusCreated, structs.NewDeck(deck))
}
...
func OpenDeck(w http.ResponseWriter, r *http.Request) {
	...
	deck, err := repo.GetDeckById(input.Id)
	if err != nil {
		if repo.IsErrInvalidId(err) {
			c.Error(http.StatusUnprocessableEntity, err)
		} else if repo.IsErrDeckNotFound(err) {
	...
	c.JSON(http.StatusOK, structs.NewDeckWithCards(deck))
}
...
func DrawCard(w http.ResponseWriter, r *http.Request) {
	...
	cards, err := repo.DrawFromDeckById(input.Id, input.Count)
	if err != nil {
		if repo.IsErrInvalidId(err) ||
			repo.IsErrDeckRemainingExceeded(err) {
			c.Error(http.StatusUnprocessableEntity, err)
		} else if repo.IsErrDeckNotFound(err) {
			c.Error(http.StatusNotFound, err)
		} else {
	...
	c.JSON(http.StatusOK, structs.NewCards(cards))
}
